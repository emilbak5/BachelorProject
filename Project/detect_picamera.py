  ...
  with picamera.PiCamera(
    ...
    try:
      ...
      for _ in camera.capture_continuous(
        ...
        if perceptron_network.person_detected == True:
            if middle_xy[0] != 0:
                object_width = middle_xy[2]
                distance_to_target = perceptron_network.getDistanceToPerson(object_width)
                
                new_motor_speeds = perceptron_network.followTarget(middle_xy, distance_to_target)
                motorSpeedLeft(1, round(perceptron_network.motor_speed_left))
                motorSpeedRight(1, round(perceptron_network.motor_speed_right))
